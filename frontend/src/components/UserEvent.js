...
const UserEvent = ({ userId }) => {
  ...
  useEffect(() => {
    ...
    let filteredEventsByWebsite =
      activeTab === "Following" ? filteredUserEvents : filteredUpcomingEvents;
    if (startechFilter) {
      filteredEventsByWebsite = filteredEventsByWebsite.filter(
        (event) => event.websiteName.toLowerCase() === "startech",
      );
    }
    if (ryanscomputersFilter) {
      filteredEventsByWebsite = filteredEventsByWebsite.filter(
        (event) => event.websiteName.toLowerCase() === "ryanscomputers",
      );
    }
    ...
    activeTab,
    startechFilter,
    ryanscomputersFilter,
  ]);

  const handleEventUnFollow = async (eId) => {
    try {
      const response = await axiosPrivate.delete(
        `/users/${userId}/events/${eId}/unfollow`,
      );
      setUserEvents((prevEvents) =>
        prevEvents.filter((event) => event.eId !== eId),
      );
      if (new Date(response.data.event.date) > new Date())
  ...
  const handleEventFollow = async (eId) => {
    try {
      const response = await axiosPrivate.get(
        `/users/${userId}/upcomingevents/${eId}/follow`,
      );
      ...
      <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 w-5/6 mt-0">
        {filteredEvents.map((event) => (
          ...
          >
            ...
            <p className="text-l text-blue-500 font-bold mb-2">
              {event.websiteName}
            </p>
