...
const AdminReview = () => {
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      await Promise.all(
        selectedReviews.map((review) => handleDelete(review.reviewId)),
      );
    } catch (error) {
    ...
    try {
      await Promise.all(
        selectedReviews.map((review) =>
          handleApprove(review.reviewId, review.userId),
        ),
      );
    } catch (error) {
  ...
  return (
    <div>
      <h2 className="text-lg font-bold mb-2 mt-10 border-b-4 border-blue-500">
        Pending Reviews
      </h2>
      <div className="flex justify-between items-center mb-4">
        <div>
          ...
          <button
            ...
            disabled={
              reviews.filter((review) => review.isSelected).length === 0
            }
          >
