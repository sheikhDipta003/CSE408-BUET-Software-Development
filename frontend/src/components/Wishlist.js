...
const Wishlist = ({ userId }) => {
  ...
  const handleDelete = async (userId, wishlistId) => {
    try {
      ...
      setWishlist((prevWishlist) =>
        prevWishlist.filter((wishlist) => wishlist.wishlistId !== wishlistId),
      );
    } catch (error) {
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      await Promise.all(
        selectedWishlist.map((wishlist) =>
          handleDelete(userId, wishlist.wishlistId),
        ),
      );
    } catch (error) {
      ...
      <div className="flex justify-between items-center mb-4">
        <div>
          ...
          <button
            ...
            disabled={
              wishlist.filter((wishlist) => wishlist.isSelected).length === 0
            }
          >
        ...
        <table className="table-auto w-full border-collapse border border-gray-200">
          <thead>
            <tr>
              ...
              >
                Website Name
                {sortBy === "websiteName" && sortOrder === "asc" && (
                  <FontAwesomeIcon
                    icon={faSortUp}
                    className="ml-2 text-black"
                  />
                )}
                {sortBy === "websiteName" && sortOrder === "desc" && (
                  <FontAwesomeIcon
                    icon={faSortDown}
                    className="ml-2 text-black"
                  />
            ...
            {filteredWishlist.length === 0 ? (
              ...
              filteredWishlist.map((wishlist) => (
                <tr key={wishlist.wishlistId}>
                  ...
                  <td className="border border-gray-200 px-4 py-2">
                    {wishlist.websiteName}
                  </td>
                  <td className="border border-gray-200 px-4 py-2">
                    {wishlist.price}
                  </td>
                  <td className="border border-gray-200 px-4 py-2">
                    {new Date(wishlist.dateAdded).toLocaleDateString("en-US", {
                      month: "long",
                      day: "numeric",
                      year: "numeric",
                    })}
